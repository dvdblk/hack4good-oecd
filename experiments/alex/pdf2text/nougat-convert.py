...
Note:   This script is based on the original script from the Nougat repository and only slightly
        modified to ignore some corrupted files.
"""
import argparse
import logging
import os
...
def main():
    ...
    for pdf in args.pdf:
        ...
        try:
            ...
            logging.info(f"PdfStreamError: Could not load file {str(pdf)}.")
            continue
        except TypeError:
            logging.info(f"TypeError: Could not load file {str(pdf)}.")
            continue
